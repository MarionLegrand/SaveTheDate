...
export class AuthentificationProvider {
  ...
  login(mail: string, mdp: string): Observable<number> {
    /*Hary-Marion remplacer 192.168.1.10 par 192.168.1.10*/
    return this.http.get('http://192.168.1.10/Al2cServer-war/webresources/utilisateur/authentification?mail=' + mail + '&mdp=' + mdp)
  ...
  synchroniserContacts(): Observable<boolean> {
    ...
    return this.http.get('http://192.168.1.10/Al2cServer-war/webresources/utilisateur/synchroniserContacts?token=' + Number(localStorage.getItem('token')))
      .map(res => {
