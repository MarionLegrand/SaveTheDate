...
export class AuthentificationProvider {
  ...
  login(mail: string, mdp: string): Observable<number> {
    /*Hary-Marion remplacer 2.6.10.86 par 2.6.10.86*/
    return this.http.get('http://2.6.10.86/Al2cServer-war/webresources/utilisateur/authentification?mail=' + mail + '&mdp=' + mdp)
  ...
  synchroniserContacts(): Observable<boolean> {
    ...
    return this.http.get('http://2.6.10.86/Al2cServer-war/webresources/utilisateur/synchroniserContacts?token=' + Number(localStorage.getItem('token')))
      .map(res => {
