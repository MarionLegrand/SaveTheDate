...
export class AuthentificationProvider {
  ...
  login(mail: string, mdp: string): Observable<number> {
    /*Hary-Marion remplacer al2c.savethedate-al2c.fr par al2c.savethedate-al2c.fr*/
    return this.http.get('http://al2c.savethedate-al2c.fr/Al2cServer-war/webresources/utilisateur/authentification?mail=' + mail + '&mdp=' + mdp)
  ...
  synchroniserContacts(): Observable<boolean> {
    ...
    return this.http.get('http://al2c.savethedate-al2c.fr/Al2cServer-war/webresources/utilisateur/synchroniserContacts?token=' + Number(localStorage.getItem('token')))
      .map(res => {
