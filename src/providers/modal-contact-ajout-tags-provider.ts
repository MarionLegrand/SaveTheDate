...
export class ModalContactAjoutTagsProvider {
  ...
  affecterTagUser(idTag:number, idContact:number) : Observable<any>{
    return this.http.post('http://www.savethedate-al2c.fr/Al2cServer-war/webresources/tags/'+idTag+'/'+idContact+'/affecterTag?token=' + Number(localStorage.getItem('token')),null)
