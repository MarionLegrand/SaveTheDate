 ...
 */
  getUserData(): Observable<userAccueil> {
    return this.http.get('http://www.savethedate-al2c.fr/Al2cServer-war/webresources/utilisateur/afficherParametres?token=' + Number(localStorage.getItem('token')))
      .map(res => {
 ...
 */
  getEventsAVenirData(): Observable<[EventAbstract]> {
    return this.http.get('http://www.savethedate-al2c.fr/Al2cServer-war/webresources/evenements/getListeEvenementsA_Venir?token=' + Number(localStorage.getItem('token')))
