 ...
 */
  getUserData(): Observable<userAccueil> {
    return this.http.get('http://192.168.1.10/Al2cServer-war/webresources/utilisateur/afficherParametres?token=' + Number(localStorage.getItem('token')))
      .map(res => {
 ...
 */
  getEventsAVenirData(): Observable<[EventAbstract]> {
    return this.http.get('http://192.168.1.10/Al2cServer-war/webresources/evenements/getListeEvenementsA_Venir?token=' + Number(localStorage.getItem('token')))
